...
// Testing
var files=[];
app.post("/upload",upload.single('file_uploaded'),function(req,res){
    db.collection('users').insertOne(req.body,function(err,result){
     cloudinary.uploader.upload(req.file.path, function(error, result) {
        files.push(result.secure_url); 
      db.collection('users').updateOne({email:user_email},{$set : {imageurl : result.secure_url}},function(err,result){                              
                 if(err){
                   throw err;
